...
var URL = require('url'), that, _resolve;
	...
	if (R.scheme) {
		T is R, subject to path removes dot segments
	} else if (R.authority) {
		T uses B.scheme
	...
	*/
	...
	_resolve = function(base,ref) {
		var b = URL.parse(base||"",true,true), r = URL.parse(ref||"",true,true), ret = "";
		// base must always be valid
		if (!base || base === "" || !b.protocol || b.protocol === "" || ((!b.host || b.host === "") && b.protocol !== "file:")) {
			ret = null;
		} else if (r.protocol) {
			// if r.protocol === b.protocol, and no r.host, then we use b.host per http://url.spec.whatwg.org/#scheme-state step 2.4
			ret = ((r.protocol === b.protocol && !r.host) ? b.protocol + getAuth(b.auth,b.host) : r.protocol + getAuth(r.auth,r.host))
				+ resolveDots(r.path) + (r.hash||"");
		} else if (r.host) {
			ret = (b.protocol||"") + getAuth(r.auth,r.host)+resolveDots(r.path) + (r.hash||"");
		} else {
			ret = (b.protocol||"") + getAuth(b.auth,b.host);
			if (! r.pathname) {
				ret += resolveDots(b.pathname) + (r.search||b.search||"");
			} else if (r.pathname[0] === '/' ) {
				// absolute path case
				// handle the special windows drive case http://url.spec.whatwg.org
				if (b.pathname && b.pathname.length > 2 && b.pathname.match(/^\/[a-z]:/i) && !r.pathname.match(/^\/[a-z]:/i)) {
					ret += b.pathname.substring(0,3);
				}
				ret += resolveDots(r.path);
			} else {
				// relative path case

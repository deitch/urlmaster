...
var URL = require('url'), that, _resolve;
	...
	*/
	...
	_resolve = function(base,ref) {
		var b = URL.parse(base||"",true,true), r = URL.parse(ref||"",true,true), ret = "", getAuth = function(a,h) {
			var t = "";
			t += (a?a+'@':'');
			t += (h||"");
			if (t) {
		...
		if (!base || base === "" || r.protocol) {
			...
			} else if (r.pathname.substring(0,1) === '/' ) {
				// absolute path case
	...
	that = {
		...
		resolveTrack : function(base,ref) {
			var ret;
			base = base || "";
			ref = ref || "";
			...
			if (typeof(base) === "string") {
				if (typeof(ref) === "string") {
					ret = [base,ref,_resolve(base,ref)];
				} else {
					ret = [];
					ref = [].concat(ref||"");
					// resolve multiple ref for a single base
					ref.forEach(function(r){
						ret.push([base,r,_resolve(base,r)]);
					});
				}
			} else {
				base = [].concat(base||"");
